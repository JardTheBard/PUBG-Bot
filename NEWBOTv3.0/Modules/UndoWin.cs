...
{
    public class UndoWin : ModuleBase<SocketCommandContext>
    {
        [Command("undowin")]
        public async Task UndoWinAsync([Remainder] string userName = "")
        {
            ...
            switch (Context.User.Id)
            {
                ...
                case 175755733395046400: //Colin
                    userData = UserDataHandling.WriteUserWins(userName, userData);
                    ...
                    else if (userData[0] == "recordedData")
                    {
                        builder.WithTitle(userName)
                            .WithDescription($"Value updated. This user now has {userData[1]} wins.")
